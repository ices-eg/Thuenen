...
drop table if exists commercial_sample_process.DiscFi_split;
create table commercial_sample_process.DiscFi_split
...
SELECT
 ...
 FROM
  (SELECT
	...
	FROM
			commercial_sample_final.DiscFi mf
...
--		mf.GESAMTKG >0	
 ...
 union 
   (select 
		...
		0 as GESAMTKG,
		0 as GESAMTSTCK,
		sw.discard_component as UPKG,
		sum(sl.discard_length_total) as UPSTCK
	...
	where	
		sw.discard_component IS NOT NULL 
	and
