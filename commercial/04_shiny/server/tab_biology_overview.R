...
select_year <- reactive({
        subset(haul_fo, year==input$year, select=c(year,quarter,ha_index))
    })
    ...
    select_quarter <- reactive({
        if (input$quarter=="All"){
            ...
            filter(select_year(), quarter %in% input$quarter)
        }
    ...
    select_weight <- reactive({
        we_index <- subset(sample_weight, ha_index %in% select_quarter()$ha_index)
    })
