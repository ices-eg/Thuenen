...
ggplot(snglior_short, aes(laenge, totalgew))+ geom_point(data=subset(snglior_short, fish=="293"), fill="grey70", colour = "black", alpha =0.7) +  
  labs(x = "LÃ¤nge (cm)", y = "Gewicht (g)") + facet_grid(quartal ~subdiv) +
  stat_summary(data=subset(snglior_short, fish=="607"), aes(group=subdiv), fun.y=mean, geom="line", colour="red") +
  theme(text = element_text(size=20))


# Einzelkomponenten fÃ¼r ln_a und b vorbereiten
snglior_short <- snglior_short %>% mutate("sx"= log(laenge*10), "sy"=log(totalgew), "sxy" = sx*sy, "sqx"=sx*sx, "sqy"=sy*sy)
