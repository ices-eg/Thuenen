# -*- coding: utf-8 -*-
"""
Created on Tue gear_mapOct  6 13:40:30 2020
...
class Frm_Gear(QWidget, frm_gear.Ui_frm_gear):
    ...
    def show_data(self):
        ...
        if self.gear_names == "":
            ...
            sql_statement = "SELECT * FROM gear_map;"

            cur.execute(sql_statement)

            self.gear_prop = cur.fetchall()
            cur.close()
            self.gear_names = [gear[1] for gear in self.gear_prop]
            self.cb_netz.addItems(self.gear_names[2:])
            index = self.cb_netz.currentIndex()
            for i in range(13, len(self.buttons)):
                self.buttons[i].setEnabled(int(self.gear_prop[index + 1][i - 11]))
        ##########################
        ...
        sql_statement = """SELECT * FROM haul_gear WHERE
