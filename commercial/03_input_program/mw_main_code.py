...
class MainWindow(QMainWindow, mw_main.Ui_mw_Main):
    ...
    def account_activated(self):
        ...
        if ref.connection != None:
            ...
            sql_data = "set search_path to com_new_final,public;"
    ...
    def load_activities(self):
        ...
        sql_data = """SELECT ha_index, haul FROM haul_fo WHERE
    ...
    def new_cruise_added(self):
        ...
        in the SQL database"""
        if not hasattr(self.nc_dlg, 'cruise_num') or self.nc_dlg.stateWin is 0:
            return
        cur = ref.connection.cursor()
        cur.execute("SELECT MAX(tr_index) from trip;")
        self.cruise_uid = cur.fetchone()[0] + 1

        trip_comm = """INSERT INTO trip (tr_index, year, trip_number, eunr, 
    ...
    def new_haul_added(self):
        ...
        if not hasattr(self.nh_dlg, 'ha_index'):
            return
        ...
        cur = ref.connection.cursor()
        cur.execute("SELECT MAX(ha_index) from haul_fo;")
